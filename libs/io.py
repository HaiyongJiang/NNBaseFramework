...
def export_pointcloud_details(vertices, normals, scale, out_file, as_text=True):
    # vertices
    assert(vertices.shape[1] == 3)
    scales = np.ones((normals.shape[0],1), dtype=np.float32)*float(scale)
    vertices = np.concatenate([vertices, normals, scales], axis=1)
    vertices = vertices.astype(np.float32)
    vertices = np.ascontiguousarray(vertices)
    vector_dtype = [('x', 'f4'), ('y', 'f4'), ('z', 'f4'), ('nx', 'f4'), ('ny', 'f4'), ('nz', 'f4'), ("value", 'f4')]
    vertices = vertices.view(dtype=vector_dtype).flatten()
    plyel = PlyElement.describe(vertices, 'vertex')
